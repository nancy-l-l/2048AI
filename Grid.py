...
"""
  def minimax(self, grid, depth, alpha, beta, us):
        ...
        if us:
            maxEval = float('-inf')
            for move in moves:
                eval_val, _ = self.minimax(move[1], depth-1, alpha, beta, not us)
                if eval_val > maxEval:
                    maxEval = eval_val
                    best_move = move[0]
            ...
            for move in moves:
                eval_val, _ = self.minimax(move[1], depth-1, alpha, beta, not us)
                avg+=eval_val
            minEval = avg//len(moves)
            beta=min(beta, minEval)
            if beta<=alpha:
                self.heuristic(grid), None
            #if 1 in moves and 2 in moves:
                #moves=[t for t in moves if t[0] != 1]
